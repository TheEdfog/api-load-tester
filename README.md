...
Параметры `--max-error-rate` и `--max-p95-ms` задают условия для автоматической проверки. JSON-отчёт сохраняется при любом результате, а код возврата будет ненулевым, если хотя бы один порог превышен.
